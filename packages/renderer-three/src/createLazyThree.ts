import type { RendererAdapter, SceneEntity, ViewportState } from "@handscroll/core";

/**
 * Dual-canvas Three overlay. Does **not** import `three` until `ensureLoaded()`.
 * Engine only calls that when a model3d entity (or a three-backed plugin) needs WebGL 3D.
 */
export function createLazyThreeRenderer(): RendererAdapter {
  return new LazyThreeAdapter();
}

interface InnerThree {
...
class LazyThreeAdapter implements RendererAdapter {
  ...
  private inner: InnerThree | null = null;
  private loadPromise: Promise<void> | null = null;
  private width = 1;
  private height = 1;
  private dpr = 1;
  private lastViewport: ViewportState | null = null;
  private entities: readonly SceneEntity[] = [];
  ...
  async ensureLoaded(): Promise<void> {
    ...
    if (!this.loadPromise) {
      this.loadPromise = (async () => {
        ...
        inner.setSceneEntities(this.entities);
        this.inner = inner;
      })();
    }
  ...
  setSceneEntities(entities: readonly SceneEntity[]): void {
    ...
    this.inner?.setSceneEntities?.(entities);
  }
